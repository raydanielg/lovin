...
<div class="text-center mb-6">
    <h2 class="text-xl font-semibold text-white mb-1">Create an account</h2>
...
<form method="POST" action="{{ route('register') }}" class="space-y-4">
    ...
    <!-- Name Field -->
    <div>
        <label for="name" class="input-label @error('name') error @enderror flex items-center gap-1 mb-1.5">
            Full Name <span class="text-red-500">*</span>
        </label>
        <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <i class="fas fa-user text-sm @error('name') error-icon @else text-gray-400 @enderror"></i>
            </div>
            <input id="name" type="text" name="name" value="{{ old('name') }}" 
                class="auth-input w-full pl-10 pr-3 py-2.5 rounded text-sm @error('name') error @enderror" 
    ...
    <div>
        ...
        <div class="relative">
            ...
            <button type="button" onclick="togglePassword('password', 'password-toggle-icon-1')" 
                class="password-toggle absolute inset-y-0 right-0 pr-3 flex items-center">
                <i id="password-toggle-icon-1" class="fas fa-eye-slash text-sm"></i>
            </button>
        </div>
        @error('password')
            <p class="error-text">{{ $message }}</p>
